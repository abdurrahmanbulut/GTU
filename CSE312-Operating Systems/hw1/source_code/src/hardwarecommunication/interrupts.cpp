...
myos::common::uint32_t myos::hardwarecommunication::InterruptManager::timer_interrupt_count = 0;
...
//         //Retrieve the value of the ESP register, which is the stack pointer, and store it in the esp variable. This is done using the mov instruction.
//         asm("mov %%esp, %0" : "=r"(esp));
//         ActiveInterruptManager->TimerInterrupt(esp);
//     }
//     //Add a value of 0x20 to the stack pointer, which is done using the add instruction.
//     asm("add $0x20, %esp");
//     //Return from the interrupt using the iret instruction.
//     asm("iret");
// }
