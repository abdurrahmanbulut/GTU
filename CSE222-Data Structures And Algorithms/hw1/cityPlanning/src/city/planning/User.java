...
public class User implements IUser, IEditMode, IViewMode{
    ...
    public void totalRemainingLengthOfLands(Street street) {
        MyArray<Structure> tempLeft = street.getLeftSideBuildings();
        MyArray<Structure> tempRight = street.getRightSideBuildings();
        ...
        for (int i = 0; i < tempLeft.getSize(); i++) {
            Structure temp = (Structure) tempLeft.getArrayElement(i);
            usedLandLength += temp.getLength();
        }
        for (int i = 0; i < tempRight.getSize(); i++) {
            Structure temp = (Structure) tempRight.getArrayElement(i);
            usedLandLength += temp.getLength();
        }
    ...
    public void skylineSilhouette(Street street) {
        ...
        for (int i = 0; i < streetLen; i++) {
            highestBuild=0;
            for (int j = 0; j < tempLeft.getSize(); j++) {
                Structure temp = (Structure) tempLeft.getArrayElement(j);
                try{
            ...
            for (int j = 0; j < tempRight.getSize(); j++) {
                Structure temp = (Structure) tempRight.getArrayElement(j);
                try{
                    if (i <= temp.getPositionEnd() && i > temp.getPositionBegin()){
                        if (highestBuild < temp.getHeight())
                            highestBuild = temp.getHeight();
